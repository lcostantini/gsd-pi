...
describe('gsd-recover', async () => {
  ...
  test('handleRecover explains a missing PLAN behind an orphaned task SUMMARY', async () => {
    // Regression: an orphaned SUMMARY (parent slice/task never registered via
    // its own PLAN) throws LegacyImportClassificationError with code
    // LEGACY_IMPORT_CLASSIFICATION_LIFECYCLE_AUTHORITY_INVALID and only
    // target_key context -- handleRecover previously let this propagate as a
    // bare, unexplained message ("legacy import canonical lifecycle has no
    // hierarchy row"). It must now name the missing milestone/slice/task and
