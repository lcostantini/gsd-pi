...
/**
 * Baseline formatting every structured legacy-import error gets, with no
 * exceptions: class name, stage, code, and the error's own context/evidence
 * as readable key:value lines. Previously every LegacyImport*Error that
 * wasn't individually special-cased fell through to a bare `err.message`
 * with no code, stage, or context at all — exactly the "gsd recover failed:
 * legacy import canonical lifecycle has no hierarchy row" style of
 * unhelpful error this and later commits fix. This deliberately does not
 * include a stack trace: the goal is to surface what the error itself
 * already carries (structured, safe-to-show data), not internal file/line
 * detail.
 */
